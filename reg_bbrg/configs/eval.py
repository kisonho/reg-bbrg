...
class EvalConfigs(_Configs):
    ...
    @classmethod
    def from_training(cls, cfgs: TrainingConfigs) -> Self:
        """
        Create an evaluation configuration from training configurations.

        - Parameters:
            - cfgs: The training configurations in `c2b2.configs.TrainingConfigs`.
        - Returns: A new evaluation configuration
        """
        return cls(batch_size=cfgs.batch_size, c_lambda=None, comments=cfgs.comments, data_dir=cfgs.data_dir, devices=cfgs.devices, experiment=cfgs.experiment, replace_experiment=cfgs.replace_experiment, fast_sampling=False, model=cfgs.output_model, show_verbose=cfgs.show_verbose, time_steps=None, use_multi_gpus=cfgs.use_multi_gpus, vqgan_path=None)
    ...
    def get_arguments(parser: argparse.ArgumentParser | argparse._ArgumentGroup = argparse.ArgumentParser()) -> argparse.ArgumentParser | argparse._ArgumentGroup:
        ...
        testing_args.add_argument("--disable_fast_sampling", action="store_true", default=False, help="A flag to disable fast sampling.")
        ...
        device_args.add_argument("-d", "--devices", type=str, default=None, nargs="+", help="The device(s) used for training, default is `None`.")
    ...
    def show_settings(self) -> None:
        ...
        view.logger.info(f"Diffusion model settings: fast_sampling={not self.disable_fast_sampling}, time_steps={self.time_steps}")
